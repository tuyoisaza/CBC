  ...
  return (
    <main className="min-h-screen bg-cbc-black py-24">
      <div className="mx-auto max-w-2xl px-4 sm:px-6 lg:px-8 text-center">
        <h1 className="text-4xl font-bold text-cbc-cream mb-4">Track Your Order</h1>
        <p className="text-gray-400 mb-8">Enter your order code to view its current status.</p>
      ...
      <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-[#1a1a1a] rounded-xl p-8 border border-gray-800 shadow-xl space-y-8">
          ...
          {order.cfdis.length > 0 && (
            <div>
              <h3 className="text-lg font-bold text-cbc-cream mb-4">Invoices / CFDI</h3>
                  ...
                  <div key={cfdi.id} className="flex items-center justify-between bg-cbc-black rounded-lg p-4 border border-gray-800">
                    ...
                    <div className="flex items-center gap-3">
                      <span className={`text-sm ${cfdi.status === 'valid' ? 'text-green-400' : 'text-gray-500'}`}>
                        {cfdi.status === 'valid' ? 'Valid' : cfdi.status}
                      </span>
...
export default function TrackingPageEn() {
  ...
  if (!searchCode || error) {
    return <TrackingSearch onSearch={(code) => { setSearchCode(code); window.history.replaceState({}, '', `/en/tracking/${code}`) }} />
  }
  ...
  if (loading) {
    return (
      <main className="min-h-screen bg-cbc-black py-24">
        <div className="mx-auto max-w-2xl px-4 sm:px-6 lg:px-8 text-center">
          <p className="text-gray-400">Searching order...</p>
        </div>
        <PublicFooter lang="en" />
      </main>
    )
  ...
  return (
    <TrackingResult
      ...
      onBack={() => {
        ...
        setOrder(null)
        setError('')
        window.history.replaceState({}, '', '/en/tracking')
      }}
    />
  )
}
